...
1. **If argument contains `/` OR ends with `/`:**
   ...
   - Example: `specs/archive/feature-user-auth/` → use as-is

2. **Otherwise (ticket shorthand):**
   - Resolve to: `specs/archive/{argument}/`
   - Example: `feature-user-auth` → `specs/archive/feature-user-auth/`
...
```markdown
❌ Archived ticket not found: {ticket-name}
...
- `feature-user-authentication` → "authentication"
- `feature-payment-processing` → "payment-processing"
- `enhancement-cache-optimization` → "cache-optimization"
- `bug-login-500-error` → [Bug fixes don't get feature docs, show message]

**For bugs/spikes:**
```markdown
ℹ️  This is a {bug/spike} ticket.
...
**Action types:**
- ✨ **Create** - New section (for new docs or missing sections)
- 📝 **Update** - Modify existing section (content outdated)
- ➕ **Add to** - Append to existing section (new content to add)
- ⚠️  **Verify** - Section may be incorrect (discrepancy found)
...
✅ Read {N} interface files
⏭️  Skipped {M} internal files
...
**Confirm completion:**
```markdown
✅ Documentation updated: docs/features/{feature-name}.md
...
Living documentation is now in sync! 📚
...
- ❌ Code-level comments (those belong in code)
- ❌ Obvious information
- ❌ Implementation history (use git for that)
...
1. ✅ Loads complete archived ticket context
2. ✅ Verifies implementation with smart code inspection
3. ✅ Proposes specific, targeted documentation updates
4. ✅ Presents proposals for human review (not automatic)
5. ✅ Generates accurate content from verified details
6. ✅ Provides conversational refinement when needed
7. ✅ Keeps living documentation in sync with implementations
